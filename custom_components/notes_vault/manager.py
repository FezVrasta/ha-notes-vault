...
class NotesVault:
    ...
    async def async_start(self) -> None:
        ...
        views = set(stored.get("views", []))
        if self.options[CONF_GENERATE_VIEWS]:
            views = await self.hass.async_add_executor_job(self._seed_views, views)
        await self._templates_store.async_save(
            {"seeded": sorted(seeded), "views": sorted(views)}
        )
    ...
    def _apply(
        ...
        if renames:
            ...
            self._written.clear()
        try:
            self._sync_room_pages(keep, paths)
        except (OSError, VaultError):
            _LOGGER.exception("Could not update the room pages")
        return stats
    ...
    def _backlinks(self, path: str) -> list[dict[str, str]]:
        ...
        for other in self.vault.iter_markdown():
            ...
            # Room pages link to their area only to show it; they aren't mentions.
            if other in (path, self.index_path) or other.startswith(
                f"{self.rooms_folder}/"
            ):
                continue
            try:
    ...
    def _index_entries(self) -> dict[str, list[tuple[str, str]]]:
        ...
        skip = (f"{self.templates_folder}/", f"{self.bases_folder}/")
        views = self.view_note_paths
        sections: dict[str, list[tuple[str, str]]] = {}
        for path in self.vault.iter_markdown():
            if path == self.index_path or path.startswith(skip) or path in views:
                continue
            try:
                note = self.vault.read_note(path)
            except (OSError, VaultError):
    ...
    def _render_index(self, sections: dict[str, list[tuple[str, str]]]) -> str:
        lines = [
            "# Notes index",
            "",
            "> [!note] Generated by Notes Vault",
            "> Every note with something written in it, rewritten whenever a note "
    ...
    # -- Obsidian views (Bases) ----------------------------------------------------

    @property
    def bases_folder(self) -> str:
        """Folder holding the `.base` files the view notes embed."""
        return f"{self.base}/Bases" if self.base else "Bases"

    @property
    def rooms_folder(self) -> str:
        """Folder holding the generated room pages."""
        return f"{self.bases_folder}/Rooms"

    @property
    def view_note_paths(self) -> set[str]:
        """The notes that explain and embed the views, next to the index."""
        folder = f"{self.base}/" if self.base else ""
        return {f"{folder}{name}" for name in DEFAULT_VIEW_NOTES}

    def _render_view(self, text: str, area: str = "") -> str:
        return render_view(
            text,
            bases=self.bases_folder,
            templates=self.templates_folder,
            index=self.index_path,
            area=area,
        )

    def _seed_views(self, seeded: set[str]) -> set[str]:
        """Write the default views and their notes once. Runs in the executor.

        Like the templates: written once and never overwritten, since Obsidian saves
        column widths and sorting into the `.base` file itself. A view the user
        deletes stays deleted; removing the Bases folder brings them all back.
        """
        if not self.vault.exists(self.bases_folder):
            seeded = set()
        folder = f"{self.base}/" if self.base else ""
        files = {
            **{f"{self.bases_folder}/{n}": t for n, t in DEFAULT_BASES.items()},
            **{f"{folder}{n}": t for n, t in DEFAULT_VIEW_NOTES.items()},
        }
        for path, text in files.items():
            name = PurePosixPath(path).name
            if name not in seeded and not self.vault.exists(path):
                self.vault.write_text(path, self._render_view(text))
            seeded.add(name)
        return seeded

    def _sync_room_pages(
        self, docs: dict[DocKey, Doc], paths: dict[DocKey, str]
    ) -> None:
        """Keep one page per area, embedding the room view. Runs in the executor.

        The pages are views, not notes: rewritten whenever they differ, renamed with
        their area and removed with it. They're found by `ha_type: room`, which the
        index of generated notes ignores.
        """
        folder = self.rooms_folder
        existing: dict[str, str] = {}
        if self.vault.exists(folder):
            for path in self.vault.iter_markdown(folder):
                try:
                    fm = self.vault.read_frontmatter(path)
                except (OSError, VaultError):
                    continue
                if fm.get("ha_type") == KIND_ROOM and isinstance(fm.get("ha_id"), str):
                    existing[fm["ha_id"]] = path
        wanted: dict[str, tuple[str, str]] = {}
        if self.options[CONF_GENERATE_VIEWS]:
            for key, doc in docs.items():
                if doc.kind != KIND_AREA or key not in paths:
                    continue
                link = wikilink(paths[key], doc.name)
                body = self._render_view(ROOM_PAGE_BODY, area=link)
                frontmatter = {
                    "ha_type": KIND_ROOM,
                    "ha_id": doc.ha_id,
                    "area": link,
                ...
                wanted[doc.ha_id] = (
                    f"{folder}/{doc.stem}{MARKDOWN_SUFFIX}",
                    render_note(frontmatter, body),
                )
        for area_id, path in existing.items():
            if area_id not in wanted or wanted[area_id][0] != path:
                self.vault.delete(path)
        for path, text in wanted.values():
            try:
                current = self.vault.read_text(path)
            except VaultError:
                current = None
            if current != text:
                self.vault.write_text(path, text)
